...
}
{
  ...
  // 핵심은 "응답이 끝나기 전에 소리가 시작된다" 는 것. 첫 인사말처럼 짧은
  // 조각은 다음 문장과 합쳐지지만, 그래도 마지막 델타를 기다리지는 않는다
  say(
    firstAt >= 0 && firstAt < deltas.length - 1,
    `응답이 끝나기 전에 첫 소리가 나간다 (델타 ${firstAt}/${deltas.length - 1})`,
  );
}
...
{
  const out = stream(['"정말 반가워요!" ', "라고 해요."]);
  say(out[0] === '"정말 반가워요!"', "닫는 따옴표까지 같이 가져간다");
}

// 짧은 조각은 혼자 안 보낸다 — 조각 하나가 TTS 요청 하나라, 왕복 지연이
// 그대로 문장 사이 침묵이 된다
{
  const out = stream(["네. ", "그럼 오늘은 뭐 했어요?"]);
  say(out.length === 1, `짧은 조각은 다음 문장과 합친다 (${out.length}개)`);
  say(out[0] === "네. 그럼 오늘은 뭐 했어요?", `합쳐진 결과: ${out[0]}`);
}
{
  // 합칠 다음 문장이 없으면 flush 가 내보낸다. 잃으면 안 된다
  const out = stream(["네."]);
  say(out.join("") === "네.", "짧아도 응답이 끝나면 나간다");
}
{
  const out = stream(["그건 정말 대단하네요?! ", "어떻게 했어요?"]);
  say(out[0] === "그건 정말 대단하네요?!", "연속 문장부호를 쪼개지 않는다");
}
