...
// ── 파밍 방지 ──────────────────────────────────────────────────────
//
// 이 앱의 학습 모드는 전부 "앱이 결과를 보고하면 서버가 XP 를 준다" 모양이다
// (채점이 앱에 있으니 다른 수가 없다). 금액은 서버가 정하므로 한 번에 큰 값을
// 가져갈 수는 없지만, **같은 요청을 반복 재생**하면 횟수는 막을 게 없었다.
// 완료 요청 하나를 캡처해서 루프로 돌리면 XP 가 무한히 올라가고, 리그 주간
// XP = UserStats.xpEarned 합계라 그대로 리그 1등과 보석 보상으로 이어졌다.
//
// 그래서 콤보 보너스·리그 챌린지가 쓰던 장치(쿨다운 + 하루 횟수)를 나머지
// 모드에도 깐다. 그리고 그 아래에 **하루 XP 상한**을 마지막 방어선으로 둔다 —
// 어느 엔드포인트가 새든 하루에 이 값 이상은 못 가져간다.

/**
 * 한 유저가 하루에 받을 수 있는 XP 상한.
 *
 * 기준: DIAMOND 리그 유지 XP 가 주 7,000 (하루 1,000). 아주 열심히 하는
 ...
 * 읽기 레슨(READING_REPEAT_XP_RATE)과 같은 눈금이다.
 */
export const LESSON_REPLAY_XP_RATE = 0.3;

/**
 * 같은 레슨을 이 시간 안에 또 "완료" 로 보고하면 XP 0.
 *
 * 17문항짜리 레슨을 60초 안에 두 번 끝낼 수는 없다. 정상 플레이는 걸리지
 * 않고, 요청을 반복 재생하는 루프만 걸린다.
 */
export const LESSON_REPLAY_COOLDOWN_SEC = 60;

/**
 * 연습 완료(complete-practice) 사이 최소 간격 / 하루 최대 횟수.
 *
 * 리그 챌린지에는 있었는데 연습에만 빠져 있었다. 연습은 questionIds 만 보내면
 * 되고 그 id 들이 서버가 낸 문제인지 확인할 방법이 없어서, 한 요청에 117 XP 를
 * 네트워크 속도만큼 반복할 수 있었다.
 *
 * 60회면 정상적으로 하루에 할 수 있는 연습보다 한참 위다.
 */
