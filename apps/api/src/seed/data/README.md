...
## 문법 문제 트랙 (`data/grammar-track`)

`getRoadmap(category='grammar')` 이 읽는 별도 로드맵. 어휘 트랙과 구조는 같지만
`gt_` 접두사로 code 가 갈려서 서로 안 섞인다.

```
노드 = 교재 1과   →  레슨 = 문법 1개   →  레슨당 문제 20개
                                        (grammar_blank 10 + grammar_build 10, 번갈아)
```

- 레슨의 `grammarCode` 는 `data/grammar/grammar.data.ts` 의 문법 code 와 **반드시**
  일치해야 한다. 안 맞으면 문제를 틀렸을 때 띄울 설명이 없다.
  (`seed:validate-grammar` 가 대조한다)
- 문제 키는 `gp_s{섹션}_u{유닛}_g{문법번호}_{01..20}`. 01~10 이 빈칸, 11~20 이 조립.
  `mix20()` 이 둘을 번갈아 섞는다. **한 번 정한 키는 바꾸지 않는다** — 오답 장부가 끊긴다.
- 문법 하나가 문제 20개를 못 채우면 문법을 쪼개지 말고 문항을 더 쓴다.
  레슨 = 문법 1개라는 규칙이 깨지면 로드맵에서 뭘 배우는지 안 보인다.
...
`full` 안에서 `answer` 를 찾아 앞뒤로 잘라 `sentencePrefix` / `sentenceSuffix` 를 만든다.

- **`answer` 는 `full` 안에 그대로 있어야 한다.** 없으면 빈칸이 문장 맨 앞에 생긴다.
- 빈칸은 문장에 **하나만**. 두 개 이상은 `blankAnswers` 를 쓰는 다른 타입이다.
- 빈칸에 넣는 건 그 레슨의 문법이어야 한다. 어휘를 묻는 빈칸은 어휘 트랙 몫이다.

### `grammar_build` — 조립

문장을 조각으로 쪼개서 자리마다 보기 3개를 준다. 정답을 순서대로 이으면 문장이 된다.
...
build(
  ...
  G1,
  [
    { options: ['민수는', '민수은', '민수를'], correct: '민수는' },
    { options: ['한국', '가족', '곧'], correct: '한국' },
    { options: ['사람', '사람이', '사람을'], correct: '사람' },
    {
      options: ['이에요.', '예요.', '입니다.'],
      correct: '이에요.',
      glue: true,
...
한 조각이 길면 문법을 고르는 게 아니라 문장을 통으로 외우는 문제가 된다.
`한국 사람이에요.` 같은 12자짜리 보기는 딱 봐도 정답이라 아무것도 안 가르친다.

- **조사는 앞말에 붙여 둔다.** `민수 | 씨는` 이지 `씨 | 는` 이 아니다.
  조사만 떼면 한국어 어절이 아닌 게 화면에 뜬다.
- **서술격만 떼어낸다.** `이에요` `예요` `입니다` `입니까` `이세요` `아니에요`.
  이게 문법 트랙에서 실제로 묻는 부분이다.
- **띄어쓰기는 그대로 조각 경계.** `한국 사람이에요.` → `한국 | 사람 | 이에요.`
- **동사·형용사는 쪼개지 않는다.** `공부했 | 어요` 는 앞이 단어가 아니다.
- 조각은 **6자 이하**를 목표로. 넘으면 대개 안 쪼갠 어절이 남아 있다.

**`glue` — 안 쓰면 정답 문장이 깨진다**

`build()` 는 `correct` 를 이어서 `answer` 와 `answerTranslation.ko` 를 만든다.
그냥 이으면 `"저는 학생 이에요."` 가 된다. 한 어절을 쪼갠 **뒷조각에 `glue: true`**
를 붙이면 앞말에 공백 없이 붙어 원문 띄어쓰기가 살아난다.

- 띄어쓰기로 나뉜 조각엔 붙이지 않는다. (`한국` 앞에는 공백이 맞다)
- 첫 조각엔 의미 없다.
- 화면에서 조립 중인 문장을 그릴 때도 같은 규칙을 쓴다 (`joinPicks`).

**오답 만드는 기준 — 조각 종류마다 다르다**

오답이 아무 말이나 되면 `했어요.` 옆에 `했어요가.` 가 뜨고, 학습자는 이상하게
생긴 걸 피하는 법만 배운다. 조각이 무슨 자리인지에 맞춰 만든다.

| 조각    | 예                                | 묻는 것             |
| ------- | --------------------------------- | ------------------- |
| 서술격  | 이에요 ← 예요 / 입니다            | 높임·격식           |
| 조사    | 커피를 ← 커피을 / 커피는          | 조사 짝 (받침 규칙) |
| 부사격  | 집에서 ← 집에 / 집까지            | 자리 조사           |
| 명사    | 사람 ← 사람이 / 사람을            | 조사가 붙나 안 붙나 |
| 서술어  | 했어요. ← 찾았어요. / 들어갔어요. | 뜻 (어미는 같게)    |
| 관형형  | 먹을 ← 가는 / 읽는                | 뜻                  |
| 지시어  | 제 ← 저 / 내                      | 인칭·높임           |
| 날짜·수 | 5월 ← 4월 / 6월                   | 숫자                |
| 나머지  | 찾으러 ← 보러 / 먹으러            | 어휘 (어미는 같게)  |

- **서술어끼리는 어미를 맞춘다.** `먹어요.` 의 오답은 `읽어요.` 지 `먹었습니다.` 가
  아니다. 어미가 다르면 뜻이 아니라 모양만 보고 고른다.
- **오답도 실제 한국어여야 한다.** 예외는 조사 짝(`저은`, `사람를`) — 이건 받침
  규칙을 틀리게 만든 거라 오답인 게 문법적으로 의미가 있다.
- 보기는 정답 포함 **3개, 서로 달라야 한다.**
- 조각을 쪼갤 때 헷갈리기 쉬운 것들:
  `작지만` 은 명사+`만` 이 아니다 · `가는` `좋은` `먹을` 은 명사+조사가 아니라 관형형 ·
  `샤워하고` 는 명사+`하고` 가 아니라 동사 · `같이` `많이` 는 명사+`이` 가 아니다.

**`hints` — 틀렸을 때 한 줄**

```ts
{ options: ['저는', '저은', '저를'], correct: '저는',
  hints: { 저은: "받침 없는 말 뒤에는 '는'", 저를: '주어 자리엔 목적격을 못 쓴다' } }
```

선택지 문자열이 키다. 비워두면 화면이 `sentenceBuild.wrongHere` 문구를 대신 쓴다.
자동 문구는 "여기가 틀렸어" 수준이라 규칙을 못 가르친다 — **조사·서술격 오답에는
채우는 게 좋다.**

### 다 쓰고 나서

```bash
pnpm --filter api seed:validate-grammar    # grammarCode ↔ 문법 데이터 대조
pnpm --filter api seed:grammar-track       # 문법 트랙만 시딩 (code 기준 upsert)
```

`seed:grammar-track` 은 `seed` 와 달리 안 쓰는 옛 문항을 지우지 않는다. 문제 키를
바꿔서 버려진 문항이 생기면 손으로 지워야 한다.
...
| `seed:validate-grammar`     | 문법 페이지 받침 규칙·퀴즈 정답 수·하이라이트, 문법 트랙 `grammarCode` |
