...
import * as section2 from './data/vocabulary/section2';
  ...
  for (const les of node.lessons) {
    ...
    for (const k of keys) {
      ...
      if (['fill_in_blank'].includes(q.type)) {
        if (!q.sentenceTemplate && !q.options?.includes(q.answer))
          err.push(`${k}: answer '${q.answer}' 가 options 에 없다`);
        if (
          !q.sentenceTemplate &&
          (q.options?.length ?? 0) < q.answer.split(' ').length + 2
        )
          warn.push(
        ...
        if (q.sentenceTemplate) {
          ...
          if ((q.blankAnswers?.length ?? 0) !== n)
            err.push(
              `${k}: 빈칸 ${n}개인데 blankAnswers 는 ${q.blankAnswers?.length ?? 0}개`,
            );
          for (const b of q.blankAnswers ?? [])
            if (!q.options?.includes(b))
              err.push(`${k}: blankAnswers '${b}' 가 options 에 없다`);
        }
      }
...
if (SKIPPED.length) {
  console.log(
    `(아직 안 고친 섹션2 유닛 ${SKIPPED.join(', ')} 는 검사에서 뺐다)`,
  );
}
