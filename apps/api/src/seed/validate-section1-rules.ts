...
import * as section1 from './data/vocabulary/section1';
...
const L4 = (v: any) =>
  v &&
  ['ko', 'uz', 'en', 'ru'].every(
    (l) => typeof v[l] === 'string' && v[l].length > 0,
  );
const L4Options = (v: any) =>
  v &&
  ['ko', 'uz', 'en', 'ru'].every((l) => Array.isArray(v[l]) && v[l].length > 0);
for (const node of NODES) {
  for (const les of node.lessons) {
    ...
    for (let i = 1; i < keys.length; i++) {
      const a = Q[keys[i - 1]]?.type,
        b = Q[keys[i]]?.type;
      if (a && a === b)
        err.push(`${tag}: ${keys[i - 1]} → ${keys[i]} 같은 타입(${a}) 연속`);
    }
    ...
    for (const k of keys) {
      ...
      if (TYPING_TYPES.includes(q.type) && node.unit < TYPING_FROM_UNIT)
        err.push(
          `${k}: 타이핑 타입(${q.type})은 유닛 ${TYPING_FROM_UNIT}부터 — 아직 자판을 못 친다`,
        );
      if (q.type === 'reply_builder' && node.unit < REPLY_BUILDER_FROM_UNIT)
        err.push(
          `${k}: reply_builder 는 유닛 ${REPLY_BUILDER_FROM_UNIT}부터 — 상대 발화를 못 알아듣는다`,
        );

      // 4개 언어
      if (!L4(q.instruction)) err.push(`${k}: instruction 4개 언어 누락`);
      if (!L4(q.hint)) err.push(`${k}: hint 4개 언어 누락`);
      if (q.explanation && !L4(q.explanation))
        err.push(`${k}: explanation 4개 언어 누락`);
      const needsTr = !['word_matching', 'audio_match'].includes(q.type);
      if (needsTr && !L4(q.answerTranslation))
        err.push(`${k}: answerTranslation 4개 언어 누락`);

      // 매칭류: 단어만 + 5쌍
      if (['word_matching', 'audio_match'].includes(q.type)) {
        if (q.pairs?.length !== 5)
          err.push(`${k}: pairs ${q.pairs?.length}쌍 (5쌍이어야 함)`);
        for (const p of q.pairs ?? []) {
          if (p.korean.includes(' '))
            err.push(`${k}: pairs 에 띄어쓰기 있는 '${p.korean}' — 단어만`);
          if ([...p.korean].length > 5)
            err.push(`${k}: '${p.korean}' 너무 길다 (5자 이하)`);
        }
      }
      ...
      if (['fill_in_blank'].includes(q.type)) {
        if (!q.options?.includes(q.answer))
          err.push(`${k}: answer '${q.answer}' 가 options 에 없다`);
        if ((q.options?.length ?? 0) < q.answer.split(' ').length + 2)
          warn.push(
            `${k}: 오답 distractor 가 적다 (options ${q.options?.length})`,
          );
        if (q.sentenceTemplate)
          err.push(`${k}: 유닛1은 단일 빈칸만 (sentenceTemplate 금지)`);
      }
      if (q.type === 'dialog_complete') {
        if (!q.options?.includes(q.answer))
          err.push(`${k}: answer 가 options 에 없다`);
        for (const [i, l] of (q.dialogLines ?? []).entries()) {
          if (!['npc', 'user'].includes(l.speaker))
            err.push(`${k}: dialogLines[${i}].speaker 이상`);
          if (/^[가-힣A-Za-z]+\s*:/.test(l.text))
            err.push(
              `${k}: dialogLines[${i}] 에 화자 이름이 박혀 있다 — 화면이 배지를 그린다`,
            );
        }
        const last = q.dialogLines?.[q.dialogLines.length - 1];
        if (last?.speaker !== 'npc')
          err.push(`${k}: 마지막 줄이 npc 여야 한다 (빈 자리가 user 차례)`);
      }
      if (q.type === 'image_choice') {
        if (!q.choices?.some((c: any) => c.text === q.answer))
          err.push(`${k}: answer 가 choices 에 없다`);
        if (q.choices?.[0]?.text === q.answer)
          warn.push(`${k}: 정답이 첫 번째 — 순서 섞어라`);
      }
      ...
      if (
        ...
        if (need.length < 2)
          err.push(`${k}: 정답이 한 덩어리다 — 조립할 게 없다`);
        const miss = need.filter((w: string) => !q.options?.includes(w));
        if (miss.length)
          err.push(`${k}: options 에 [${miss}] 없어 '${q.answer}' 조립 불가`);
        ...
        for (const [w, n] of Object.entries(needCnt)) {
          ...
          if (have < n)
            err.push(
              `${k}: '${w}' 가 정답에 ${n}번 나오는데 options 엔 ${have}개뿐 — 칩이 모자란다`,
            );
        }
        if ((q.options?.length ?? 0) <= need.length)
          err.push(`${k}: 오답 어절이 없다`);
        if (q.options?.slice(0, need.length).join(' ') === q.answer)
          err.push(`${k}: options 가 정답 순서 그대로다`);
      }
      ...
      if (q.type === 'sentence_builder' && node.unit === 1) {
        ...
        if (!L4Options(q.optionsI18n))
          err.push(`${k}: optionsI18n 4개 언어 누락`);
        ...
        for (const lang of ['ko', 'uz', 'en', 'ru']) {
          ...
          if (nativeNeed.length < 2) {
            err.push(
              `${k}: answerI18n.${lang} 이 한 덩어리다 — 현지어 조립 불가`,
            );
          }
          ...
          for (const [word, count] of Object.entries(nativeCnt)) {
            const have = nativeOptions.filter(
              (option: string) => option === word,
            ).length;
            if (have < count) {
              err.push(
                `${k}: optionsI18n.${lang} 에 '${word}' 칩 부족 (${have}/${count})`,
              );
            }
          }
          ...
          if (
            nativeOptions.slice(0, nativeNeed.length).join(' ') === nativeAnswer
          ) {
      ...
      if (q.type === 'translate_builder') {
        if (q.npcText)
          err.push(
            `${k}: translate_builder 의 npcText 는 화면에 안 나온다 — instruction 으로 옮겨라`,
          );
        const shared = keys.some(
          (o) =>
            o !== k &&
            Q[o]?.type !== 'translate_builder' &&
            Q[o]?.instruction?.ko === q.instruction?.ko,
        );
        if (shared)
          err.push(
            `${k}: instruction 이 공용 지시문이다 — 옮길 문장을 4개 언어로 적어야 한다`,
          );
        if (q.instruction?.ko?.includes(q.answer)) {
          err.push(
            `${k}: instruction.ko 에 정답이 그대로 들어 있다 — 베끼게 된다`,
          );
        }
      }
      ...
      if (q.type === 'reply_builder' && !q.npcText) {
        err.push(
          `${k}: reply_builder 에 npcText 가 없다 — 대답할 상대 발화가 있어야 한다`,
        );
      }
      ...
      if (q.type === 'word_arrange' && q.npcText) {
        err.push(
          `${k}: word_arrange 에 npcText 를 두면 정답 대신 이게 읽힌다 — 지워라`,
        );
      }
      ...
      if (q.type === 'speaking') {
        ...
        if ([...q.answer].length > 16)
          err.push(`${k}: speaking 문장이 길다 (${[...q.answer].length}자)`);
...
const used = new Set(
  NODES.flatMap((n) => n.lessons.flatMap((l: any) => l.questions)),
);
const orphans = Object.keys(Q).filter((k) => !used.has(k));
if (orphans.length)
  err.push(
    `레슨에 안 들어간 문제 ${orphans.length}개: ${orphans.slice(0, 5)}...`,
  );
...
for (const unit of UNITS) {
  ...
  for (const n of nodes) {
    if (n.lessons.length !== 4) {
      err.push(
        `유닛 ${unit} 노드 ${n.order}: 레슨 ${n.lessons.length}개 (4개 고정)`,
      );
    }
  }
  ...
  const qs = nodes.reduce(
    (a, n) =>
      a + n.lessons.reduce((b: number, l: any) => b + l.questions.length, 0),
    0,
  );
  console.log(
    `유닛 ${unit}  노드 ${nodes.length} · 레슨 ${lessons} · 문제 ${qs}`,
  );
}
