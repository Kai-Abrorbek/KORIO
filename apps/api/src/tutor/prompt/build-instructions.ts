...
export function buildTutorInstructions(
  ...
  const lines: string[] = [
    ...
    `- SPEAK ONLY KOREAN OUT LOUD. Your ${native} pronunciation is not good enough to`,
    `  be useful to a learner, and hearing it teaches them nothing.`,
    `- When they need a ${native} explanation, keep SPEAKING Korean and put the`,
    `  ${native} in one short phrase they will read on screen. Say the Korean`,
    `  sentence around it naturally.`,
    `- Your written ${native} must read like a native speaker wrote it, never like`,
    `  machine translation.`,
    `- If they speak to you in ${native}, understand it, give the Korean way to say`,
    `  it, and lead them back to Korean.`,
    `  Example: they say something in ${native} -> "아, 그건 한국어로 '액션 영화를 봤어요' 예요. 한번 말해볼까요?"`,
    `- If they are completely lost after two tries, it is better to simplify your`,
    `  Korean than to switch languages.`,
    ``,
    // ── 수준 ──
