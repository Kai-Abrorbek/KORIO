import topik from "./topik/en";

export default {
  topik,
  common: {
    loadFailed: "Couldn't load. Please try again in a moment",
    retry: "Try again",
    start: "Get Started",
    next: "Next",
    back: "Back",
    confirm: "Confirm",
    cancel: "Cancel",
    save: "Save",
    loading: "Loading...",
    close: "Close",
  },
  rank: {
    bannerTitle: "Where do I rank?",
    bannerSub: "See your place among every learner",
    prefix: "#",
    suffix: "",
    topPercent: "Top {{p}}%",
    ofTotal: "out of {{total}} learners",
    tier: {
      legend: "Legend",
      master: "Master",
      elite: "Elite",
      rising: "Rising",
      steady: "Steady",
      starter: "Starting out",
    },
    breakdown: {
      proficiency: "Level",
      volume: "Volume",
      consistency: "Streak",
    },
    levelValue: "Level {{level}}",
    xpValue: "{{xp}} XP",
    dayValue: "{{days}} days",
    nextRank: "{{xp}} XP to move up one place",
    nextRankMaxed: "Above this, level and streaks decide it",
    first: "First overall. Nowhere left to climb",
    unranked: {
      title: "No rank yet",
      desc: "Finish one lesson and you'll take your place among {{total}} learners.",
    },
    error: "Couldn't load your rank · {{code}}",
  },
  tour: {
    next: "Next",
    done: "Start",
    skip: "Skip",
    replay: "Replay the feature tour",
    home: {
      continue: {
        title: "Start today's lesson here",
        desc: "This one button is all you need — it remembers where you stopped and picks up there.",
      },
      rank: {
        title: "Check your rank",
        desc: "Tap to see where you stand among every learner. Ranks move constantly, so the result shows for a minute.",
      },
      categories: {
        title: "Switch subject",
        desc: "Pick what you feel like today: vocabulary, grammar, phrases, listening or TOPIK.",
      },
      chart: {
        title: "This week's study",
        desc: "Colours show what you worked on and how much. Tap for the full breakdown.",
      },
      review: {
        title: "Redo just your mistakes",
        desc: "Every question you got wrong lands here. Get it right twice and it leaves the list.",
      },
      shop: {
        title: "Where your gems go",
        desc: "Spend gems on energy and passes. Everything you own is here.",
      },
      ai: {
        title: "Ask whenever you're stuck",
        desc: "Any Korean you can't work out — ask right here, any time.",
      },
    },
  },
  navTabs: {
    home: "Home",
    stats: "Stats",
    league: "League",
    premium: "Premium",
  },
  splash: {
    subtitle: "Learn Korean with AI",
    hasAccount: "I already have an account",
  },
  onboarding: {
    survey: {
      title: "Why do you want to learn Korean?",
      subtitle: {
        selfLevel: "Be honest!",
        hangul: "Let's check your Hangul first",
        goals: "A goal gets you there faster",
        interests: "Let's learn with what you love",
        style: "Your way of learning",
        daily: "A little every day works best",
        reminder: "I'll help you not forget",
      },
      helper: {
        single: "Choose one option",
        multiple: "Choose as many as you like",
      },
      start: "Start",
      selfLevel: {
        title: "How much Korean do you know?",
        completeBeginner: "Total beginner",
        basicGreetings: "I know basic greetings",
        basicConversation: "I can have simple conversations",
        above: "More than that",
      },
      hangul: {
        title: "Can you read Hangul?",
        none: "Not yet",
        partial: "A little",
        fluent: "Yes, fluently",
      },
      interests: {
        title: "What would make learning fun?",
        kpop: "K-pop",
        drama: "K-drama",
        travel: "Travel",
        business: "Business",
        topik: "TOPIK exam",
        game: "Games",
        food: "K-food",
      },
      reminder: {
        title: "When should we remind you?",
        morning: "Morning",
        afternoon: "Afternoon",
        evening: "Evening",
        skip: "No reminders",
      },
      goals: {
        kpop: "K-pop / K-drama",
        travel: "Travel",
        work: "Work / Business",
        study: "Academic study",
        friend: "Korean friend / partner",
        interest: "Just love languages",
      },
      style: {
        title: "How would you like to learn?",
        grammar: "Structured grammar",
        conversation: "Conversation focused",
        game: "Game-like and fun",
        vocabulary: "Vocabulary focused",
      },
      daily: {
        title: "How much time can you study daily?",
        five: "5 minutes",
        ten: "10 minutes",
        fifteen: "15 minutes",
        twenty: "20+ minutes",
      },
    },
    levelTest: {
      title: "Let's check your level!",
      subtitle: "We'll assess your level with 10 questions",
      start: "Start test",
    },
    result: {
      eyebrow: "PLACEMENT COMPLETE",
      title: "We found your perfect starting point",
      subtitle:
        "Skip lightly over what you know and begin with the Korean you need now.",
      recommendationLabel: "Your learning path",
      personalized: "For you",
      sectionLabel: "SECTION",
      recommendationTitle: "Section {{section}} is the right place to start",
      recommendationBody:
        "We chose this point from your test answers. You can revisit earlier sections whenever you want.",
      unlocked: "Section {{section}} is ready",
      reviewOpen: "Every earlier section is open for review",
      firstSectionReady: "Build a strong foundation from the first step",
      correct: "Correct answers",
      score: "Accuracy",
      loginNote: "Log in to save this starting point to your account.",
      startAtSection: "Start at Section {{section}}",
      seePlan: "See my plan",
    },
  },
  auth: {
    login: "Log In",
    register: "Sign Up",
    email: "Email",
    password: "Password",
    nickname: "Nickname",
    noAccount: "Don't have an account?",
    hasAccount: "Already have an account?",
    or: "or",
    forgot: {
      link: "Forgot your password?",
      title: "Forgot your password?",
      subtitle: "Tell us the email you signed up with and we'll send a reset code.",
      hint: "If you signed up with Google or Kakao there is no password — just use that button to sign in.",
      cta: "Send code",
    },
    verify: {
      title: "Enter the code",
      subtitle: "We sent a 6-digit code to {{email}}.",
      cta: "Continue",
      resend: "Send a new code",
      resendIn: "You can ask for a new code in {{count}}s",
      resent: "Code sent again",
      spam: "Can't find it? Check your spam folder too.",
    },
    reset: {
      title: "Choose a new password",
      subtitle: "Pick something you don't use anywhere else.",
      newPassword: "New password",
      confirm: "Repeat new password",
      cta: "Change password",
      mismatch: "The two passwords don't match",
      tooShort: "Use at least 6 characters",
      logoutNote: "Changing your password signs you out on every other device.",
      strength: { weak: "Weak", fair: "Fair", strong: "Strong" },
    },
    errors: {
      UNKNOWN_ERROR: "Something went wrong. Please try again in a moment.",
      INVALID_CODE: "That code doesn't match. Please check it again.",
      TOO_MANY_ATTEMPTS: "Too many wrong tries. Request a new code.",
      INVALID_RESET_TOKEN: "This verification expired. Please start over.",
      TOO_MANY_REQUESTS: "Too many requests. Please try again in a moment.",
      INVALID_CREDENTIALS: "Invalid email or password.",
      EMAIL_ALREADY_EXISTS: "Email already in use.",
      USER_NOT_FOUND: "Account not found.",
      SOCIAL_LOGIN_FAILED: "Social login failed. Please try again.",
    },
  },
  home: {
    streak: "day streak",
    today: "Today",
    calendar: "Calendar",
    continueLesson: "Continue Learning",
    longestStreak: "Longest streak",
    continueLessonBtn: "Continue",
    reviewRate: "Review accuracy",
    vocabStudy: "Vocabulary",
    mode: {
      vocabulary: "Vocabulary",
      grammar: "Grammar",
      grammarPractice: "Grammar drills",
      expression: "Expression",
      speaking: "Speaking",
      conversation: "Conversation",
      listening: "Listening",
      topik: "TOPIK",
      pronunciation: "Pronunciation",
    },
    dailyGoal: "Daily goal",
    weeklyStudy: "This week",
    weeklyInfo: "Study Info",
    weekUnit: "week",
    studyTime: "hr",
    month: "Month",
    studyMin: "min",
    studyProblems: "problems",
    thisWeek: "This week",
    categories: {
      vocab: "Vocab",
      grammar: "Grammar",
      expression: "Expression",
      conversation: "Conversation",
      listening: "Listening",
      topik: "TOPIK",
      other: "Other",
      pronunciation: "Pronunciation",
    },
    review: "Review",
    reviewSub: "Wrong answers · Practice",
    shop: "Shop",
    challenge: "Challenges",
    dictionary: "Dictionary",
    wordbook: "My Wordbook",
    days: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
    streakDays: "Day streak",
    repairPen: "Repair pens",
    medalTitle: "Monthly medals",
  },
  update: {
    upToDate: "You're on the latest version",
    outdated: "v{{version}} is available",
    current: "Current",
    noNotes: "No release notes recorded.",
    footer:
      "We're still making KORIO better. Send us your thoughts from the help center!",
    tags: { new: "New", improve: "Improved", fix: "Fixed" },
    notes: {
      v1_2_400: {
        items: [
          "new|Pronunciation practice rebuilt around 120 Korean minimal pairs.",
          "new|Account management and help center screens.",
          "new|Sound settings: volume, speaking speed and vibration controls.",
          "improve|Your learning mode and TOPIK level are stored on your account, so they follow you to a new phone.",
          "fix|The answer was visible before you solved a grammar drill. Fixed.",
        ],
      },
      v1_2_300: {
        items: [
          "new|Grammar drills moved onto the roadmap, with the same XP and rewards.",
          "new|Notification center — friends, league and rewards in one place.",
          "improve|The check button no longer gets hidden on lesson screens.",
        ],
      },
      v1_2_200: {
        items: [
          "new|Added the Jamo Slot game.",
          "improve|The word match game now uses words you've actually learned.",
          "fix|SUPER members were still losing energy. Fixed.",
        ],
      },
      v1_2_100: {
        items: [
          "new|Score screen shows your progress section by section.",
          "improve|Complete beginners now start from the Hangul node on the roadmap.",
          "fix|Cards were hard to see in dark mode. Fixed.",
        ],
      },
    },
  },
  games: {
    title: "Games",
    subtitle: "Review the words you've learned by playing. Games earn XP too!",
    hangulTitle: "Hangul games",
    hangulDesc: "Letter games live inside Hangul practice",
    items: {
      matchGame: {
        name: "Word match",
        desc: "Pair each word with its meaning, fast",
      },
      wordMemory: {
        name: "Word memory",
        desc: "Flip the cards and find the pairs",
      },
      wordChain: {
        name: "Word chain",
        desc: "Take turns linking words with Haneulmon",
      },
      wordRain: {
        name: "Word Rain",
        desc: "Catch the meaning before the word hits the ground",
      },
      swipeJudge: {
        name: "Swipe Judge",
        desc: "Swipe to judge if the word and meaning match",
      },
      particleRush: {
        name: "Particle Rush",
        desc: "Fill in the right particle before time runs out",
      },
      echoChain: {
        name: "Echo Chain",
        desc: "Remember and tap the words in the order you hear",
      },
    },
  },
  help: {
    searchPlaceholder: "Search for an answer",
    categories: {
      all: "All",
      learning: "Learning",
      premium: "Subscription",
      account: "Account",
      etc: "Other",
    },
    noResult: "No answer found",
    noResultDesc:
      "Try different words, or scroll down and contact us directly.",
    contactSection: "Contact us",
    contactSubject: "Question",
    email: "Email us",
    telegram: "Telegram",
    telegramDesc: "Fastest way to reach us",
    replyNote:
      "We usually reply within 1-2 days. Your app version and device info are attached automatically.",
    aboutSection: "Legal",
    terms: "Terms of service",
    privacy: "Privacy policy",
    faq: {
      energy: {
        q: "How does energy refill?",
        a: "Energy refills on its own over time. You can also top it up with gems, or claim a free refill up to three times a day. With KORIO SUPER there's no energy limit at all.",
      },
      streak: {
        q: "I lost my streak",
        a: "Skipping even one day resets the streak to zero. A repair pen can bring back a missed day. Turn on the daily reminder in Settings so it doesn't happen again.",
      },
      xp: {
        q: "How do I earn XP?",
        a: "You earn XP every time you finish a lesson. Answering correctly in a row adds a combo bonus, and reviews and games count too. XP decides your league ranking.",
      },
      league: {
        q: "How do I move up a league?",
        a: "You compete on XP earned during the week against others in your league. At the end of the week the top players move up a tier and the bottom ones move down. We'll notify you of the result.",
      },
      hangul: {
        q: "I can't read Hangul at all",
        a: "That's fine. The very first node on the roadmap teaches Hangul — letters, stroke order and games, step by step. You can come back to it any time.",
      },
      levelTest: {
        q: "Can I retake the level test?",
        a: "Yes. Use the level test banner on the home screen. You can also pass a jump test on the roadmap to skip ahead to a later unit.",
      },
      superWhat: {
        q: "What does KORIO SUPER give me?",
        a: "No energy limit, so you can study as long as you want, plus locked courses and extra practice. The Premium screen lists everything.",
      },
      trial: {
        q: "Is there a free trial?",
        a: "Signing up gives you 7 days of KORIO SUPER for free. When the trial ends your account goes back to normal automatically — nothing is charged.",
      },
      cancel: {
        q: "How do I cancel my subscription?",
        a: "Payment goes through the App Store or Google Play, so cancel it in that store's subscription settings. Deleting your KORIO account does not cancel the subscription.",
      },
      refund: {
        q: "Can I get a refund?",
        a: "Refunds follow the policy of the store that processed your payment. Request it from the App Store or Google Play, and let us know if something goes wrong.",
      },
      password: {
        q: "I forgot my password",
        a: "Use the password reset on the login screen. If you signed up with Google or Telegram there's no password at all — just sign in the same way.",
      },
      device: {
        q: "Will my progress survive a new phone?",
        a: "Yes. Progress is stored on your account. Sign in on the new device and everything continues. If you're using guest mode, create an account first.",
      },
      deleteAccount: {
        q: "Does deleting my account erase everything?",
        a: "Yes — progress, streak, gems and friends are permanently deleted. You can delete your account in Settings > Account.",
      },
      offline: {
        q: "Can I use the app offline?",
        a: "Not yet — lessons are loaded from our server, so you need a connection. Offline study is on the way.",
      },
      bug: {
        q: "I found a bug",
        a: "Tell us through one of the channels below. Include which screen you were on, what you tapped, and a screenshot — it makes the fix much faster.",
      },
    },
  },
  account: {
    profileSection: "Profile",
    accountSection: "Account info",
    subscriptionSection: "Subscription",
    securitySection: "Security",
    dangerSection: "Danger zone",
    nickname: "Nickname",
    nicknamePlaceholder: "Enter a nickname",
    username: "Username",
    usernamePlaceholder: "Not set yet",
    usernameHint: "Lowercase letters, numbers, underscore — 3 to 20",
    usernameError: {
      format: "Use 3-20 lowercase letters, numbers or underscores.",
      taken: "That username is already taken.",
    },
    noUsername: "No username",
    bio: "Bio",
    bioPlaceholder: "Say something about yourself",
    email: "Email",
    loginMethod: "Sign-in method",
    providers: {
      local: "Email",
      google: "Google",
      telegram: "Telegram",
      kakao: "Kakao",
      naver: "Naver",
    },
    joinedAt: "Joined",
    superOn: "KORIO SUPER active",
    superOff: "KORIO SUPER",
    superUntil: "Until {{date}}",
    superCta: "See what's included",
    changePassword: "Change password",
    socialNoPassword: "Social account — no password to change",
    currentPassword: "Current password",
    newPassword: "New password",
    newPasswordAgain: "Confirm new password",
    pwRule: "At least 6 characters",
    pwTooShort: "Must be at least 6 characters",
    pwMismatch: "Passwords don't match",
    pwWrong: "Current password is wrong",
    pwSame: "That's your current password",
    pwChanged: "Password changed",
    logout: "Log out",
    deleteAccount: "Delete account",
    deleteAccountDesc: "Your account and all progress will be erased",
    deleteTitle: "Delete your account?",
    deleteWarning:
      "Your progress, streak, gems and friends will be permanently deleted. If you have a subscription, cancel it separately in the store.",
    deleteConfirmLabel: "Type '{{name}}' to confirm",
    saveFailed: "Couldn't save. Please try again.",
  },
  comingSoon: {
    badge: "In progress",
    title: "Coming soon!",
    desc: "We're building a dedicated practice page for this mode. Hang tight!",
    pickOther: "Pick another mode",
  },
  roadmap: {
    comingSoonTitle: "Coming soon!",
    comingSoonDesc: "This course is still being prepared. Stay tuned!",
    sectionUnit: "Section {{section}}, Unit {{unit}}",
    courseTab: "Courses",
    newCourses: "New Courses",
    enrolledTitle: "Learning",
    scoreExplain: "About score",
    allSections: "All sections",
    sectionProgress: "{{done}} / {{total}} units done",
    sectionDone: "All {{total}} units done",
    sectionReview: "{{total}} units done · tap to review",
    sectionViewing: "Viewing",
    noSections: "No sections yet",
    sectionLockedHint: "Take a test to jump straight here",
    scoreText: "Your score is {{score}}.",
    jumpTitle: "Jump to Unit {{unit}} and prove your skills!",
    jumpDescription: "Pass this test to jump straight here!",
    jumpStart: "Start test",
    jumpLater: "Maybe later",
    review: "Review",
    legend: "Legendary",
    legendSubtitle: "Reach Legendary level to prove your skills.",
    sectionN: "Section {{section}}",
    nextSection: "Next section",
    jumpHere: "Jump here?",
    lessonProgress: "Lesson {{current}}/{{total}}",
    continueWithXp: "Continue +{{xp}} XP",
    continue: "Continue",
    locked: "Locked",
    lockedDescription: "Complete all previous levels to unlock this level!",
    chestLocked: "Complete {{count}} more lessons to open this gift box!",
    chestHint: "Finish this stretch to earn gems!",
    chestReady: "You have a reward waiting!",
    chestOpen: "Open chest",
    chestTake: "Collect",
    chestMulti: "Opened {{count}} chests at once",
    chestGrade: { wood: "Wooden chest", silver: "Silver chest", gold: "Golden chest" },
    legendDone: "You reached Legend level on this one!",
    scoreReview: "Score {{score}} review",
    scoreDesc:
      "Finish Legend level on every level at this score to unlock the final trophy!",
    scoreLocked: "Finish this unit to unlock it.",
    goToLegend: "Go to Legend level",
  },
  lesson: {
    loadFailed: "Couldn't load this lesson",
    check: "Check",
    readingQuiz: "Read the passage and answer the question",
    wordCount: "{{count}} words",
    errorHunt: "Find the error in the sentence",
    errorHuntHint: "Tap the wrong word — one chance only!",
    errorHuntFix: "Fix it with the correct form",
    errorHuntFound: "Found it! Now let's fix it",
    clozePassage: "Fill in the blanks in the passage",
    dialogOrder: "Put the conversation in order",
    dialogOrderHint: "Tap the messages below to build the conversation",
    verbTransform: "Change to the correct form",
    combo: "Combo",
    quitTitle: "Wait, you're just 1 minute from finishing!",
    quitContinue: "Keep learning",
    quitLeave: "Quit",
    next: "Continue",
    correct: "Great job!",
    meaningShort: "Meaning:",
    wrong: "Incorrect!",
    correctAnswer: "Correct answer",
    meaning: "Correct! Meaning",
    matchPairs: "Match the pairs",
    matched: "pairs matched",
    previousWrong: "Previous mistake",
    speakingSkip: "Skip speaking exercise",
    speaking: {
      tapToSpeak: "Tap and speak",
      listening: "Listening…",
      analyzing: "Checking your pronunciation…",
      retry: "Try again",
      scoreLabel: "Pronunciation",
      heard: "Here's what I heard",
      accuracy: "Accuracy",
      fluency: "Fluency",
      completeness: "Completeness",
      noSpeech: "I didn't hear anything. Want to try again?",
      tooShort: "That was too short. Say the whole sentence",
      micDenied: "Microphone access is needed. Turn it on in Settings",
      micFailed: "Couldn't open the microphone",
      notSupportedHere:
        "The microphone isn't available here. Please use the app",
      checkFailed: "Can't check right now. Please try again in a moment",
    },
    typeHere: "Type here",
    tapWhatYouHear: "Tap what you hear",
    typeWhatYouHear: "Type what you hear",
    fillBlank: "Choose the word for the blank",
    fillBlankHint: "Select a blank, then choose the answer below",
    fillBlankOptions: "Choices",
    playFullSentence: "Listen to the full sentence",
    stopAudio: "Stop audio",
    skipListening: "Skip listening exercise",
    fillMissing: "Type the missing word",
    tapToOpenWordBank: "Tap to open the word bank",
    tapOrDrag: "Tap or drag words here",
    newWord: "New word",
    selectCorrectImage: "Select the correct image",
    listenAndSelect: "Listen and select",
    translateSentence: "Translate this sentence",
    typeAnswerTitle: "Write this meaning in Korean",
    translateTypeHint: "Express the meaning as a natural Korean sentence",
    sourceSentence: "Sentence to translate",
    toKorean: "Into Korean",
    yourKoreanAnswer: "Your Korean answer",
    enterKoreanAnswer: "Type a Korean sentence",
    clearAnswer: "Clear your answer",
    clear: "Clear",
    answerInKorean: "Write your answer in Korean",
    voiceFillsAnswer: "Speak and we'll fill in your answer",
    buildInKorean: "Build it in Korean",
    replyInKorean: "Answer in Korean",
    enterTranslation: "Enter translation",
    tapToSpeak: "Tap to speak",
    recording: "Listening...",
    hardPractice: "Hard practice",
    typeHeardEnglish: "Type what you hear",
    reviewIntro: "Now let's review the ones you missed!",
    continue: "Continue",
  },
  stats: {
    studyPath: {
      title: "Study path",
      level: "Level {{level}}",
      daysDone: "Days completed",
      ofTotal: "{{done}} / {{total}}",
      currentSection: "Section {{n}} now",
      sections: "Section progress",
      section: "S{{n}}",
      breakdown: "What you've finished",
      kinds: {
        words: "Words",
        grammar: "Grammar",
        review: "Review",
        quiz: "Practice",
        final: "Wrap-up",
      },
      examOpen: "Your level exam is unlocked",
      examPassed: "Level exam passed!",
      nextLevel: "Next: {{title}}",
      keepGoing: "Finish this level to unlock the exam",
    },
    momentum: {
      title: "Last 2 weeks",
      thisWeek: "Studied {{n}} days this week",
      thisWeekShort: "This week",
      lastWeek: "Last week",
      up: "+{{n}} days vs last week",
      down: "-{{n}} days vs last week",
      same: "Same as last week",
    },
    tabs: { period: "By Period", category: "By Category" },
    todayInfo: "Today's Study Info",
    noStudyRecord: "No study records yet.",
    emptyTodayTitle: "No record for today yet!",
    emptyTodayDesc: "Check back after completing a lesson.",
    periodInfo: "Study Info by Period",
    yearlyStudy: "Yearly Study",
    totalProblems: "Total problems",
    newProblems: "New",
    reviewProblems: "Review",
    weekdayPattern: "Usually on {{day}}?",
    weekdayDesc: "You study {{count}} problems in {{time}} on average.",
    weekdays: {
      "0": "Sunday",
      "1": "Monday",
      "2": "Tuesday",
      "3": "Wednesday",
      "4": "Thursday",
      "5": "Friday",
      "6": "Saturday",
    },
    recent: "Recent",
    studyTime: "Study Time",
    studyVolume: "Study Volume",
    studyInfo: "Study Info",
    avgStudyTimeSuffix: ".",
    avgStudyTime: {
      week: "Last week, your daily average was ",
      month: "Last month, your daily average was ",
      year: "Last year, your daily average was ",
      all: "All time, your daily average was ",
    },
    avgVolume: {
      week: "Last week, you solved an average of ",
      month: "Last month, you solved an average of ",
      year: "Last year, you solved an average of ",
      all: "All time, you solved an average of ",
    },
    avgVolumeSuffix: " per day.",
    problemsCount: "{{count}} problems",
    nDays: "{{count}} days",
    period: { week: "Week", month: "Month", year: "Year", all: "All" },
    category: {
      vocab: "Vocabulary",
      grammar: "Grammar",
      expression: "Expression",
      conversation: "Conversation",
      listening: "Listening",
      topik: "TOPIK",
      other: "Other",
    },
    radar: {
      title: "Learning balance",
      range: "Last {{days}} days",
      notYet: "Not started",
      foot: "Based on your last {{count}} questions",
      diagnosis: {
        noData: "Practice a bit more and I'll show you your strengths and weak spots.",
        balanced: "You're covering every area evenly. Keep it up!",
        weakSpot: "{{weakest}} is falling well behind your other areas.",
        untouched: "You haven't started {{category}} yet. Want to give it a go?",
        accuracyDrop: "You do a lot of {{category}}, but your accuracy is low. Time to review.",
      },
    },
    categoryTrophy: "{{category}} Trophy",
    todayStudyTime: "Today's Study Time",
    totalStudyTime: "Total Study Time",
    newWords: "New words",
    knownWords: "Known words",
    reviewWords: "Review words",
    reviewAccuracy: "Review accuracy",
  },
  courses: {
    chooseLanguage: "Which language will you learn?",
    chooseLanguageSub: "Pick a language to get started",
    fieldCount: "{{count}} areas",
    languages: { korean: "Korean", english: "English" },
    chooseCategory: "What will you study?",
    modeTitle: "How do you want to learn?",
    modeGuidedNote: "Places you can open on their own, outside the path we plan for you.",
    modeFreeNote: "Pick anything you like and jump in.",
    modeHint: "Your progress is kept. You can switch back any time.",
    guidedEntryTitle: "Today's study path",
    guidedEntryDesc: "Start with today's plan, laid out in order",
    categories: {
      hangul: "Hangul",
      games: "Games",
      vocab: "Vocabulary",
      grammar: "Grammar",
      expression: "Expressions",
      conversation: "Conversation",
      listening: "Listening",
      topik: "TOPIK",
      pronunciation: "Pronunciation",
      grammarPractice: "Grammar drills",
      wordCard: "Word cards",
      speaking: "Speaking",
    },
    categoryDesc: {
      hangul: "Start with the letters",
      games: "Review by playing",
      vocab: "Learn words",
      grammar: "Master the rules",
      expression: "Common phrases",
      speaking: "Repeat sentences by topic",
      conversation: "Real dialogue practice",
      listening: "Train your ear",
      topik: "Exam prep",
      pronunciation: "Tell sounds apart",
      grammarPractice: "Blanks and sentence building",
      wordCard: "Flip cards to memorize",
    },
  },
  profile: {
    logout: "Log Out",
    joinedYear: "Joined in {{year}}",
    courses: "Courses",
    following: "Following",
    followers: "Followers",
    addFriend: "Add a friend",
    learningStatus: "Learning Status",
    streakDays: "{{count}} days",
    xp: "{{count}} XP",
    gems: "{{count}} gems",
    friendStreak: "Friend Streaks",
  },
  friendProfile: {
    followedByOne: "Followed by {{name}}",
    followedByMany: "Followed by {{name}} and {{count}} others",
    follow: "Follow",
    followBack: "Follow back",
    following: "Following",
    weeklyProgress: "Weekly Progress",
    me: "Me",
    report: "Report user",
    block: "Block",
    achievements: {
      title: "Achievements",
      explorer: "Explorer",
      champion: "Champion",
      scholar: "Scholar",
      master: "Master",
      streakKing: "Streak King",
    },
  },
  friends: {
    online: "Online",
    title: "Friends",
    findFriendsTitle: "Find friends",
    followedBy: "Followed by {{name}}",
    nameOrId: "Name or ID",
    contactsTitle: "KORIO users in your contacts",
      inviteReward: "Invite friends, earn gems",
      phoneTitle: "Find friends by your number",
      phoneDesc:
        "Add your number and we'll find friends from your contacts — and they'll be able to find you too.",
      phonePlaceholder: "Phone number",
      phoneSave: "Add and find friends",
      phonePrivacy:
        "We never store your phone number. Only a matching hash and the last 4 digits are kept.",
      phoneErr: {
        INVALID: "Please check the number.",
        TAKEN: "That number is already registered.",
      },
      openSettings: "Open settings",
      inviteTitle: "Contacts not on KORIO yet",
      inviteDesc: "Invite them and you both get {{gems}} gems.",
      invite: "Invite",
      inviteMessage:
        "{{name}}, it's {{nickname}}! Let's learn Korean together on KORIO 🇰🇷\nUse my invite code {{code}} and we both get gems.\n{{link}}",
    contactsCount: "{{count}} contacts",
    contactsDenied: "No contacts permission",
    contactsNoMatch: "No users found in your contacts",
    followAll: "Follow all",
    followMe: "Follow me on KORIO!",
    findFriends: "Find friends",
    fromContacts: "Select from contacts",
    searchByName: "Search by name",
    shareLink: "Share follow link",
    suggestions: "Suggestions",
    seeAll: "See all",
    follow: "Follow",
    followBack: "Follow back",
    following: "Following",
    tabs: {
      following: "Following",
      followers: "Followers",
    },
  },
  userCourses: {
    loadFailed: "Couldn't load this",
    title: "{{name}}'s Courses",
    list: {
      korean: "Korean",
      english: "English",
      spanish: "Spanish",
      chess: "Chess",
      japanese: "Japanese",
      chinese: "Chinese",
      russian: "Russian",
      music: "Music",
      math: "Math",
    },
  },
  notifs: {
    title: "Notifications",
    readAll: "Mark all read",
    today: "Today",
    earlier: "Earlier",
    justNow: "Just now",
    minAgo: "{{count}}m ago",
    hourAgo: "{{count}}h ago",
    yesterday: "Yesterday",
    dayAgo: "{{count}}d ago",
    emptyTitle: "No notifications yet",
    emptyDesc: "Study and make friends, and news will show up here.",
    type: {
      follow: { title: "{{nickname}} started following you", body: "" },
      league_promoted: {
        title: "Promoted to the {{toTier}} league",
        body: "Rank {{rank}} · {{gems}} gems",
      },
      league_demoted: {
        title: "Moved down to the {{toTier}} league",
        body: "Rank {{rank}} · climb back up",
      },
      league_result: {
        title: "This week's league ended",
        body: "Rank {{rank}} · {{gems}} gems",
      },
      chest: { title: "A chest is waiting", body: "Tap the chest on your roadmap to collect gems" },
      streak: {
        title: "{{days}}-day streak",
        body: "Your flame burns brighter",
      },
      streak_risk: {
        title: "Your streak is at risk",
        body: "Study today to keep it",
      },
      energy_full: { title: "Energy is full", body: "Good time to study" },
      level_up: { title: "You levelled up", body: "" },
      super_expiring: {
        title: "Your free trial is ending",
        body: "{{days}} days left",
      },
      referral: {
        title: "{{nickname}} joined with your code",
        body: "{{gems}} gems have been added",
      },
      system: { title: "{{message}}", body: "" },
    },
  },
  settings: {
    display: {
      darkModeSection: "Dark mode",
      learningSection: "Learning screen",
      lightMode: "Light mode",
      darkMode: "Dark mode",
      learningTheme: "Learning theme",
      themes: { skyBlue: "Sky blue", purple: "Purple" },
    },
    language: {
      subtitle: "Choose your app language",
    },
    notifications: {
      channelDefault: "KORIO notifications",
      channelStudy: "Study reminders",
      master: "All notifications",
      masterDesc: "Turn all notifications on or off",
      learningSection: "Learning",
      daily: "Daily reminder",
      dailyDesc: "We'll remind you at your set time",
      time: "Time",
      streak: "Streak reminder",
      streakDesc: "Get warned before your streak breaks",
      socialSection: "Competition · Social",
      league: "League updates",
      friends: "Friend activity",
      otherSection: "Other",
      events: "Events · News",
      pickTime: "Pick reminder time",
    },
    sound: {
      muteNow: "Mute now",
      muteNowDesc: "Silent for this session only",
      mutedHint: "Muted — setting previews still play.",
      volumeSection: "Volume",
      speech: "Speech",
      sfx: "Sound effects",
      key: "Key sounds",
      voiceSection: "Speech",
      voice: "Voice",
      voiceDesc: "Choose a Korean speaker",
      voicePickerTitle: "Choose a voice",
      voicePickerDesc:
        "Preview each voice, then select the one you want to hear in lessons.",
      loadingVoices: "Loading Korean voices...",
      voiceLoadError: "Couldn't load the voice list.",
      retry: "Try again",
      female: "Female",
      male: "Male",
      selectedVoice: "Selected",
      rate: "Speaking speed",
      rateSlow: "Slow",
      rateNormal: "Normal",
      rateFast: "Fast",
      preview: "Preview",
      autoPlay: "Auto play",
      autoPlayDesc: "Read each question out loud automatically",
      hapticSection: "Vibration",
      keyHaptics: "Vibrate on tap",
      keyHapticsDesc: "When you press a button or key",
      rewardHaptics: "Correct · reward vibration",
      rewardHapticsDesc: "When you answer correctly or earn a reward",
      otherSection: "Other",
      startMuted: "Start muted",
      startMutedDesc: "Open the app silently every time",
      reset: "Reset to defaults",
    },
    title: "Settings",
    version: "App version: {{version}}",
    user: {
      subscribe: "Subscribe to Premium",
      freeTooltip: "Right now!",
      manage: "Manage subscription",
      trialActive: "Free trial active!",
      trialLeftDays: "{{count}} days left",
      freeTooltipDesc: "Try {{days}} days for free!",
    },
    quickActions: {
      authCode: "Code",
      friends: "Friends",
    },
    guest: {
      title: "You're in guest mode.",
      description:
        "If you uninstall KORIO from this device, you may lose your study data! Sign up to keep it safe.",
      login: "Log in with existing account",
      signup: "Sign up",
    },
    items: {
      display: {
        title: "Display",
        description: "Dark mode, study screen theme",
      },
      notifications: {
        title: "Notifications",
        description: "Study time, report, league notifications",
      },
      sound: {
        title: "Sound & Effects",
        description: "Study audio and sound effects",
      },
      favorites: {
        title: "Favorites",
        description: "Study modes shown on home",
      },
      vocab: {
        title: "Vocabulary",
        description: "Quiz style, wordbook, difficulty",
      },
      grammar: {
        title: "Grammar",
        description: "Daily goal, audio speed and volume",
      },
      expression: {
        title: "Expression",
        description: "Settings, audio speed and volume",
      },
      conversation: {
        title: "Conversation",
        description: "Settings, audio and sound effects",
      },
      listening: {
        title: "Listening",
        description: "Mode, pronunciation, practice settings",
      },
      league: {
        title: "League",
        description: "Participation, rank change notifications",
      },
      report: {
        title: "Study Report",
        description: "Sharing and arrival notifications",
      },
      account: {
        title: "Account",
        description: "Account code, linked accounts, goals",
      },
      subscription: {
        title: "Subscription",
        description: "Restore purchases, info, coupons",
      },
      help: {
        title: "Help Center",
        description: "Support, certificates, announcements",
      },
      tourReplay: {
        title: "Replay the feature tour",
        description: "Walk through what each button on the home screen does",
      },
      language: {
        title: "Language",
        description: "Default language",
      },
      lab: {
        title: "Lab",
        description: "Features KORIO is experimenting with",
      },
      update: {
        title: "Update History",
        description: "App version: {{version}}",
      },
    },
  },
  lessonComplete: {
    share: {
      title: "Lesson complete!",
      headline: "Just finished a Korean lesson! 🔥",
      stats: "⚡ {{xp}} XP · 🎯 {{accuracy}}% accuracy · ⏱ {{time}}",
      streak: "🗓 {{days}}-day streak!",
      invite: "Come learn with me on KORIO — use my invite code {{code}} and get gems.\n{{link}}",
      plain: "Learning Korean on KORIO 🇰🇷",
    },
    title: "Lesson Complete!",
    totalXp: "Total XP",
    accuracy: "Accuracy",
    speed: "Speed",
    claim: "Claim XP",
    labels: {
      perfect: "Perfect",
      excellent: "Excellent",
      good: "Good",
      tryHarder: "Try Harder",
      fast: "Fast",
      speed: "Speed",
    },
  },
  chestReward: {
    titles: {
      common: "Common Item",
      rare: "Rare Item",
      epic: "Epic Item",
    },
    tapToUpgrade: "Tap to upgrade!",
    tap: "Tap!",
    oneChanceLeft: "1 chance left!",
    gemReward: "Gems +{{count}}",
    continue: "Continue",
  },
  syllableDraw: {
    title: "Build and Write",
    subtitle:
      "A consonant and a vowel combine into one syllable.\nLearn it by writing with your finger.",
    locked: "Clear the previous stage first",
    writeIt: "Copy the syllable exactly as shown",
    figureItOut: "What syllable do these letters make?",
    stroke: "Stroke {{current}} / {{total}}",
    redraw: "Redo this stroke",
    perfect: "Perfect!",
    good: "Nice work",
    okay: "Not bad",
    tryAgain: "Let's try again",
    levels: {
      basic: {
        title: "Consonant + vowel",
        desc: "가, 나, 다 — the easiest shape, vowel on the right",
      },
      wide: {
        title: "Vowel below",
        desc: "고, 구, 소 — the vowel drops under the consonant",
      },
      tail: {
        title: "Adding a batchim",
        desc: "옥, 움, 용 — one more consonant sits underneath",
      },
      tallTail: {
        title: "Three-part syllables",
        desc: "산, 밥, 김 — the finished syllable is hidden now",
      },
      wideTail: {
        title: "Write on a blank",
        desc: "곰, 물, 국 — no guide lines at all",
      },
      complex: {
        title: "Complex syllables",
        desc: "과, 꿈, 꽃 — compound vowels and double consonants",
      },
    },
    result: {
      accuracy: "Accuracy",
      next: "Next stage",
      retry: "Try again",
      exit: "Exit",
    },
  },

  hangul: {
    title: "Hangul Master",
    markDone: "I've learned Hangul",
    hero: {
      progress: "Progress",
      learned: "{{learned}} / {{total}} learned",
    },
    tabs: {
      consonants: "Consonants",
      vowels: "Vowels",
    },
    detail: {
      listen: "Listen",
      examples: "Example words",
      playGame: "Practice with game",
    },
    startGame: "Start game",
    game: {
      matchHint: "Match the character with its pronunciation",
      won: "Perfect!",
      timeLabel: "Time",
      movesLabel: "Moves",
      playAgain: "Play again",
      exit: "Exit",
    },
    draw: {
      title: "Draw Hangul",
      instruction: "Trace each stroke in the correct order",
      stroke: "{{current}}/{{total}} stroke",
      perfect: "Perfect!",
      good: "Great",
      okay: "Okay",
      tryAgain: "Try again",
      complete: "Complete!",
      retry: "Try again",
      clear: "Clear",
      exit: "Exit",
    },
    games: {
      memory: "Memory",
      drawing: "Draw",
      slot: "Slot",
      speed: "Speed",
      syllable: "Syllables",
    },
    slot: {
      makeThis: "Build this letter",
      stop: "STOP",
      combo: "{{count}} combo",
      gameOver: "Game over!",
      made: "Letters built",
      maxCombo: "Best combo",
      retry: "Play again",
      exit: "Exit",
    },
    memory: {
      level: "Level {{level}}",
      memorize: "Memorize the cards!",
      cleared: "Cleared!",
      timeUp: "Time's up!",
      nextLevel: "Next level",
      allCleared: "All levels done! 🏆",
      retry: "Retry",
      exit: "Exit",
    },
    wordMemory: {
      level: "Level {{level}}",
      memorize: "Memorize the cards!",
      combo: "{{n}} combo!",
      lastPair: "Last pair!",
      cleared: "Cleared!",
      timeUp: "Time's up!",
      nextLevel: "Next level",
      allCleared: "All levels done! 🏆",
      retry: "Retry",
      exit: "Exit",
    },
    speed: {
      getReady: "Get Ready",
      go: "GO!",
      score: "Score",
      combo: "Combo",
      timeUp: "Time's up!",
      finalScore: "Final Score",
      correct: "Correct",
      maxCombo: "Max Combo",
      accuracy: "Accuracy",
      bestScore: "Best: {{score}}",
      newBest: "New best!",
      playAgain: "Play Again",
      exit: "Exit",
    },
  },
  wordChain: {
    title: "Word Chain",
    nextSyllable: "Next syllable",
    typeHere: "Type in Korean...",
    hint: "Hint",
    hintTitle: "How about these?",
    win: "Victory!",
    lose: "Defeat...",
    aiSurrender: "The AI couldn't find a word!",
    noHearts: "You ran out of hearts",
    finalScore: "Score",
    turns: "Turns",
    maxCombo: "Best combo",
    playAgain: "Play again",
    exit: "Exit",
    exitTitle: "Quit the game?",
    exitConfirm: "Your progress won't be saved",
  },
  score: {
    raised: "You raised your Korean score!",
    continue: "Continue",
    explain: "Score explained",
    title: "Korean score",
    hint: "Your score goes up as your Korean improves",
    progress: "Score progress",
    detailTitle: "Look how far your Korean has come!",
    inProgress: "In progress",
    unitsProgress: "{{done}}/{{total}} units done",
    sectionFallback: "Section {{section}}",
    milestones: {
      start: "Get started",
      intro: "Introduce yourself",
      order: "Order at a restaurant",
      directions: "Give simple directions",
      reading: "Read short stories",
      tv: "Watch TV shows",
      work: "Communicate at work",
      complete: "Complete the course",
    },
  },
  matchGame: {
    title: "Match the words with the same meaning",
    continue: "Continue",
    bubble: "Nice, {{xp}} XP is pretty cool.",
    starEarned: "You earned {{xp}} XP and a star!",
    keepGoing: "Keep going to earn the next star?",
    timeUp: "Time's up!",
    finalScore: "You matched {{count}} pairs!",
  },
  energy: {
    errors: {
      NOT_ENOUGH_GEMS: "Not enough gems",
      FREE_LIMIT_REACHED: "You've used all your free refills today",
      UNKNOWN: "Please try again in a moment",
    },
    title: "Energy",
    charging: "Charging",
    timeToFull: "{{h}}h {{m}}m",
    unlimited: "Unlimited",
    freeTrial: "Free trial",
    freeTrialDo: "Try it free",
    refill: "Refill",
    widgetBoost: "Widget Boost",
    install: "Install",
    new: "NEW",
    plusFive: "Energy +5",
    watchAd: "Watch ad",
    nextRefill: "Next refill: in {{h}}h",
    modalTitle: "You need more energy to start this lesson!",
    tryFreeDays: "Try {{days}} days for free",
    noThanks: "No thanks",
    full: "Full",
    freeCount: "Free ({{n}})",
    freeDone: "Tomorrow",
  },
  // en
  tutor: {
    /** 통화 중 화면 (TutorCallScreen) */
    call: {
      progress: "Phrases {{done}}/{{total}}",
      step: "Step {{done}} / {{total}}",
      sayHi: "Say hello to get started",
      todayExpression: "Today's phrase",
      roleLabel: "Korean teacher",
      cheer: "Just a bit more!\nYou've got this!",
      tagline: "Practice Today, A Brighter Tomorrow",
      slower: "Say it slower",
      replay: "Replay",
      explain: "Explain it",
      micOff: "Mute",
      micOn: "Unmute",
      text: "Show text",
      textHide: "Hide text",
      end: "End lesson",
    },
    personality: {
      calm: "Calm",
      friendly: "Friendly",
      energetic: "Energetic",
      strict: "Strict",
      pronunciation: "Pronunciation",
      teasing: "Teasing",
    },
    /** 통화 전 설정 한 페이지 (TutorSetupScreen) */
    setup: {
      title: "AI Korean tutor",
      subtitle: "Choose how you'd like today's lesson.",
      language: {
        title: "Which language should explanations be in?",
        uz: "O'zbekcha",
        ru: "Русский",
        en: "English",
        ko: "한국어",
      },
      teacher: { title: "Teacher" },
      address: {
        title: "How should the teacher talk to you?",
        polite: "Polite",
        politeExample: "좋아요, 다시 한번 해볼까요?",
        casual: "Casual",
        casualExample: "좋아, 다시 한번 해보자!",
      },
      topic: { title: "What shall we talk about today?" },
      freeTalk: "Free talk",
      start: "Start the lesson",
    },
    teacher: {
      title: "Who will you study with today?",
      lead: "Each teacher has a different voice and way of speaking. Have a listen.",
      startWith: "Start with {{name}}",
      pickFirst: "Pick a teacher",
    },
    modes: {
      freeTalk: "Free talk",
      rolePlay: "Role play",
      lesson: "Lesson",
      pronunciation: "Pronunciation",
      review: "Review",
    },
    explain: {
      show: "🇺🇿 Show explanation",
      loading: "Preparing explanation...",
      failed: "Couldn't load the explanation",
    },
    title: "AI Speaking Tutor",
    start: "Start talking",
    end: "End conversation",
    limitReached: "You've used today's time",
    quotaLeft: "{{min}} min left today ({{limit}} min/day)",
    upsell: "Get 20 min a day with SUPER",
    upsellMax: "Get 20 min a day with KORIO MAX",
    tapToHear: "Tap to hear it",
    pickTopic: "What shall we practice today?",
    pickAnother: "Pick another topic",
    freeTalk: "Free talk",
    freeTalkBlurb: "Just chat, no set topic",
    todayExpressions: "Today's expressions",
    summary: {
      analyzing: "Going over your conversation...",
      title: "Conversation complete!",
      statTime: "Talk time",
      statSpoke: "You spoke",
      statWords: "New phrases",
      didWell: "You said this well",
      betterWay: "Say it this way next time",
      newWords: "Phrases from today",
      reviewHint: "Tap to hear the correct pronunciation",
      again: "Talk again",
      done: "Done",
    },
    expressionCount: "{{count}} expressions",
    topicGroup: { korea: "Living in Korea", daily: "Everyday talk" },
    state: {
      idle: "Ready when you are.",
      connecting: "Connecting...",
      listening: "Listening",
      thinking: "Thinking...",
      speaking: "Speaking",
      error: "Something went wrong",
    },
    err: {
      generic: "Something went wrong. Please try again in a moment.",
      MIC_PERMISSION_DENIED: "Microphone access is needed. Please allow it in Settings.",
      CONNECTION_LOST: "The connection dropped. Please start again.",
      CONNECTION_ERROR: "There's a problem with the connection.",
      TUTOR_DAILY_LIMIT_REACHED: "You've used today's time. See you tomorrow!",
      TUTOR_MONTHLY_LIMIT_REACHED: "You've used this month's time.",
      TUTOR_NOT_CONFIGURED: "This isn't available right now.",
      TUTOR_AGENT_UNAVAILABLE: "The teacher isn't responding. Please try again in a moment.",
      TUTOR_SESSION_FAILED: "Couldn't start the conversation. Please try again.",
    },
  },
  // Friend invite rewards (app/invite.tsx)
  invite: {
    loadFailed: "Couldn't load your invite. Please try again in a moment.",
    heroTitle: "Invite a friend — {{gems}}💎 each",
    heroSub:
      "When a friend joins with your code, you both get {{gems}} gems.",
    myCode: "My invite code",
    tapToShare: "Tap to share",
    copy: "Copy code",
    copied: "Copied",
    openInvite: "See my invites",
    invitedCount: "Friends invited",
    gemsEarned: "Gems earned",
    milestonesTitle: "Invite bonuses",
    nextMilestone: "{{n}} more for +{{gems}}💎",
    haveCode: "Got a friend's code?",
    haveCodeDesc: "Enter your friend's invite code and get {{gems}} gems now.",
    codePlaceholder: "Enter code",
    apply: "Apply",
    claimedTitle: "You got {{gems}} gems!",
    claimedBody: "{{nickname}} got them too. Now go learn together!",
    joinedFriends: "Joined with your code",
    shareCta: "Send to a friend",
    shareMessage:
      "It's {{nickname}}! Let's learn Korean together on KORIO 🇰🇷\nUse my invite code {{code}} and we both get {{gems}} gems.\n{{link}}",
    terms:
      "An invite code works within {{days}} days of signing up, once per account.",
    err: {
      INVALID_CODE: "That code doesn't look right. Please check it.",
      CODE_NOT_FOUND: "No such code. Please check it.",
      SELF_REFERRAL: "You can't use your own code.",
      ALREADY_CLAIMED: "You've already used an invite code.",
      WINDOW_CLOSED: "Invite codes only work for a while after signing up.",
      CIRCULAR: "You can't reward each other by swapping codes.",
    },
  },
  // Pre-signup plan screen (app/onboarding/plan.tsx)
  plan: {
    haveInviteCode: "I have an invite code",
    codeSaved: "Code {{code}} · applied when you log in",
    codeChange: "Change",
    freeDays: "{{days}} days free",
    heroTitle: "Start now — nothing to pay",
    heroSub: "Your personalised course starts at Section {{section}}",
    timeline: {
      title: "How your free trial works",
      dayToday: "Today",
      dayN: "Day {{day}}",
      todayTitle: "SUPER unlocks completely",
      todayBody:
        "Unlimited energy and unlimited review open right away. You pay nothing today.",
      remindTitle: "Reminder before it ends",
      remindBody: "We'll remind you twice — {{first}} days and {{second}} day before it ends.",
      endTitle: "Trial ends",
      endBody: "Pick a plan only if you want to keep going.",
    },
    featuresTitle: "What opens during the trial",
    afterTrialLabel: "Plans after the trial",
    priceLater: "You can pick a plan after the trial ends.",
    terms:
      "The {{days}}-day trial is free. Nothing is charged automatically — you decide later whether to continue.",
    footerNote: "You pay nothing today",
    footerNoteWithPrice: "{{days}} days free · then {{price}}/month",
    startTrial: "Start {{days}}-day free trial",
    authBannerTitle: "{{days}}-day free trial",
    authBannerBody: "Log in and SUPER opens right away",
    close: "Close",
  },
  premiumGate: {
    title: {
      default: "This is a KORIO SUPER feature",
      grammar: "Grammar explanations need SUPER",
      expression: "Situational phrases need SUPER",
      listening: "Reading & listening need SUPER",
      topik: "TOPIK prep needs SUPER",
      tutor: "The AI voice tutor needs SUPER",
    },
    sub: "Subscribe and everything locked right now opens up.",
    perk: {
      grammar: "All grammar explanations and drills",
      expression: "Every situational phrase pack",
      conversation: "Real speaking practice with the AI tutor",
      listening: "All reading and listening passages",
      topik: "TOPIK prep for every level",
      energy: "Unlimited energy",
    },
    cta: "Subscribe and unlock",
    taster: "Try a free taste first",
    later: "Later",
  },
  premium: {
    seeAllPlans: "See all plans",
    trialBuyNote: "You have {{days}} days of trial left. Buying now charges you immediately and ends the trial.",
    totalPrice: "{{price}} total",
    hasFreeTrial: "Free trial included",
    startTrialFree: "Start free",
    restore: "Restore purchases",
    restoring: "Restoring...",
    loadingPlans: "Loading plans...",
    retry: "Try again",
    trialLeft: "{{days}} days left in your free trial",
    renewsOn: "Renews automatically on {{date}}",
    endsOn: "Active until {{date}}",
    manageInStore: "Cancel your subscription or change your payment method in Google Play subscriptions.",
    err: {
      generic: "Something went wrong. Please try again in a moment.",
      noProducts: "Plans aren't available right now. Please try again in a moment.",
      storeDisconnected: "Couldn't reach the store. Check your connection and try again.",
      platformUnsupported: "In-app purchases aren't available on this device.",
    },
    tier: { super: "SUPER", max: "MAX" },
    tierBlurb: {
      super: "Study without limits",
      max: "Speak Korean with an AI tutor every day",
    },
    includesSuper: "Everything in SUPER",
    maxOnly: "MAX only",
    heroSub: "Learn without limits with unlimited energy",
    popular: "POPULAR",
    perMonth: "/mo",
    save: "Save {{percent}}%",
    trial: "{{days}}-day free trial",
    startTrial: "Start {{days}} days free",
    subscribe: "Subscribe now",
    billedAs: "{{period}} — billed {{price}}",
    bestValue: "Best value",
    perMonthLabel: "mo",
    terms: "Cancel anytime. Subscription auto-renews.",
    activeTitle: "SUPER Member",
    activeSub: "You're enjoying all premium perks",
    plans: {
      monthly: "Monthly",
      threeMonths: "3 months",
      sixMonths: "6 months",
      yearly: "Yearly",
    },
    features: {
      unlimitedEnergy: "Unlimited Energy",
      noAds: "No Ads",
      unlimitedReview: "Unlimited Review",
      aiTutor: "Unlimited AI Tutor",
      monthlyGems: "Monthly Gems",
      advancedStats: "Advanced Stats",
      tutorTopics: "16 real-life topics",
      tutorPronunciation: "Hear examples in a native voice",
    },
    featuresDesc: {
      unlimitedEnergy: "Keep learning, no energy limits",
      noAds: "Distraction-free learning",
      unlimitedReview: "Review mistakes endlessly",
      aiTutor: "Chat freely with AI teacher",
      monthlyGems: "500 free gems every month",
      advancedStats: "Detailed learning insights",
      tutorTopics: "Cafes, clinics, interviews — things you actually need",
      tutorPronunciation: "Tap to check the correct pronunciation",
    },
  },
  // en
  league: {
    weeklyRanking: "Weekly League",
    endsIn: "Ends in {{time}}",
    promotionZone: "Top {{count}} promote",
    demotionZone: "Bottom {{count}} demote",
    needXp: "{{xp}} XP to promote!",
    inPromotion: "You're in the promotion zone! 🔥",
    daysLeft: "{{count}}d",
    boostXp: "+{{xp}} XP",
    keepXpLabel: "Stay in league",
    keepXpSafe: "{{earned}} / {{required}} XP · safe",
    keepXpDanger: "{{earned}} / {{required}} XP · you'll drop",
    you: "(You)",
    tiers: {
      bronze: "Bronze",
      silver: "Silver",
      gold: "Gold",
      sapphire: "Sapphire",
      ruby: "Ruby",
      emerald: "Emerald",
      amethyst: "Amethyst",
      pearl: "Pearl",
      obsidian: "Obsidian",
      diamond: "Diamond",
    },
    promoteZone: "Promotion",
    demoteZone: "Demotion",
    result: {
      promoteTitle: "Promoted!",
      demoteTitle: "Demoted",
      stayTitle: "This week's result",
      promoteSub: "You moved up to {{tier}} League! 🎉",
      demoteSub: "You dropped to {{tier}} League",
      staySub: "You stayed in {{tier}} League",
      demoteXpSub: "Staying in {{tier}} League needed {{required}} XP this week",
      xpShort: "{{earned}} / {{required}} XP",
      rank: "Finished #{{rank}}",
      gems: "+{{gems}} gems",
      continue: "Continue",
    },
  },
  shop: {
    premiumTitle: "Premium passes",
    premiumDesc: "Spend your gems on more KORIO SUPER. The longer the pass, the cheaper each day.",
    premiumUntil: "Premium until {{date}}",
    days: "{{n}} days",
    perDay: "{{n}} gems a day",
    save: "Save {{n}}%",
    passDone: "{{n}} more days of premium added!",
    stackLimit: "You can stack up to {{n}} days ahead. Buy again once it runs down.",
    energyTitle: "Energy",
    dayUnit: "days",
    best: "BEST",
    need: "{{n}} more",
    stackFull: "Already stacked enough",
    stackNote: "You have {{now}} days stacked (max {{max}})",
    balance: "Your gems",
    gemWorth: "1 gem = 1 so'm",
    superTitle: "KORIO SUPER",
    superPitch: "Unlimited energy · never run out",
    superActive: "Active right now",
    superManage: "Manage",
    confirmTitle: "Buy this pass?",
    confirmBody: "Spend {{gems}} gems and get {{days}} days of premium.",
    confirmOk: "Buy",
    withdrawCardNumber: "Card number",
    withdrawPayout: "You get {{n}} so'm",
    withdrawMin: "Minimum payout is {{n}} gems",
    withdrawTooBig: "You have {{n}} gems",
    passesUnavailable: "Couldn't load the passes. The server isn't responding.",
    freeLeft: "{{n}} free left",
    withdrawSection: "Cash out gems",
    withdrawTitle: "Send to a card",
    withdrawDesc: "Get your gems back on a card. Choose the card type and enter the number.",
    withdrawAmount: "Gem amount",
    withdrawMax: "Max",
    withdrawSubmit: "Request payout",
    withdrawSoon: "Payouts aren't live yet. Nothing has been submitted.",
    errors: {
      NOT_ENOUGH_GEMS: "Not enough gems.",
      ENERGY_ALREADY_FULL: "Your energy is already full.",
      ENERGY_SUPER_UNLIMITED: "SUPER has unlimited energy — no refill needed.",
      GEM_PASS_STACK_LIMIT: "You've stacked enough already. Buy again once it runs down.",
      GEM_PASS_FAILED: "Couldn't complete it. Your gems are untouched.",
      UNKNOWN_GEM_PASS: "That item doesn't exist.",
      UNKNOWN: "Please try again in a moment.",
    },
  },
  challenge: {
    types: {
      match: "Blazing Match",
      memory: "Memory Duel",
      wordRain: "Word Rain",
      swipeJudge: "True or False",
      particleRush: "Lightning Rush",
      echoChain: "Echo Chain",
      wordChain: "Word Chain Duel",
      speed: "Speed Round",
      listen: "Listening Challenge",
    },
    playsLeft: "Plays left today",
    noEnergy: "Not enough energy",
    noPlaysLeft: "That's all for today",
    notCounted: "Try again in a moment to earn XP",
    startFailed: "Couldn't start. Please tap again in a moment.",
    scoreLabel: "Score",
    headlinePre: "Earn up to",
    headlinePost: "\nand climb the leaderboard!",
    level: "Level",
    earn: "Earn",
    start: "Start",
    readyTitle: "Get ready!",
    readyDesc: "The first round is about to begin.",
    continue: "Continue",
    resultEarned: " earned!",
    resultLevelUp: " earned — you leveled up to level {{level}}!",
    matched: "Pairs matched",
    maxCombo: "Max combo",
    newRecord: "New record",
    rankUpTitle: "You moved up to #{{rank}} in {{tier}}!",
  },
  practice: {
    mistakesNeed: "You need {{n}} mistakes to start a review!",
    mistakesReady: "Review your mistakes?",
    mistakesCount: "{{n}} mistakes",
    startXp: "Start +{{xp}} XP",
    noMistakes: "No mistakes yet",
    practiceWords: "Practice words",
    wordCount: "{{n}} words",
    sort: "Sort",
    noWords: "No words learned yet",
    title: "Practice",
    perfectPronunciation: "Perfect pronunciation",
    skills: "Skills",
    speaking: "Speaking",
    listening: "Listening",
    mistakes: "Mistakes",
    words: "Words",
    story: "Story",
    conversation: "Conversation",
    videoCall: "Video call",
    soon: "Soon",
    heroStart: "Start",
    pronunciation: "Pronunciation",
    games: "Games",
    mistakesHint: "Redo just the ones you missed",
    wordsHint: "Your learned words as cards",
    speakingHint: "Pick a topic and say it out loud",
    listeningHint: "Read and listen together",
    pronunciationHint: "Polish batchim and jamo sounds",
    gamesHint: "Lock in words by playing",
  },
  wordStudy: {
    finishDay: "Today's words done!",
    eyebrow: "WORD DECK",
    title: "Word cards",
    currentRange: "Current study range",
    range: "Section {{section}} · Unit {{unit}}",
    change: "Change",
    playback: "Pronunciation playback",
    playbackAuto: "Play automatically",
    playbackAutoSub: "Hear each word as you move to its card.",
    playbackManual: "Tap to listen",
    playbackManualSub: "Play only when you tap the speaker.",
    chooseRangeEyebrow: "CHOOSE YOUR DECK",
    chooseRange: "Choose what to study",
    section: "Section",
    unit: "Unit",
    sectionNumber: "Section {{n}}",
    unitNumber: "Unit {{n}}",
    wordsCount: "{{count}} words",
    studyThisRange: "Study this range",
    core: "Core",
    meaning: "Meaning",
    example: "Example",
    examplePending: "An example sentence for this word is coming soon.",
    listenWord: "Listen to the word",
    swipeHint: "Swipe through cards",
    recallShort: "Review",
    recallBadge: "Memory practice",
    recallOfferEyebrow: "You've seen every word",
    recallOfferTitle: "Ready to recall them?",
    recallOfferDescription:
      "Check your memory by typing the {{count}} words you just studied from their meanings.",
    recallOfferStart: "Start memory practice",
    recallOfferLater: "Maybe later",
    recallCountHint: "Not tested yet",
    recallPrompt: "Recall it from the meaning",
    recallInputLabel: "Korean word",
    recallCheck: "Check answer",
    recallCorrect: "Correct!",
    recallCorrectSub: "You can move on to the next word.",
    recallWrong: "Try recalling it once more",
    recallWrongSub: "You saw this word recently. Type it again.",
    recallGuide: "Remember the Korean word",
    recallGuideSub: "Spacing and punctuation differences are accepted.",
    recallAnswer: "Answer",
    recallHint: "Show hint",
    recallHintInstruction: "Type the shown answer yourself, then check it.",
    recallContinue: "Correct! Swipe forward to continue",
    recallAnswerFirst: "Type the Korean word and check it",
    previous: "Previous word",
    next: "Next word",
    loading: "Preparing your word cards",
    loadingSub: "We are arranging the words from your selected unit.",
    loadFailed: "Couldn't load the words",
    loadFailedSub: "Check your connection and try again.",
    retry: "Try again",
    empty: "No words in this range yet",
    emptySub: "Tap the study range above and choose another unit.",
    partOfSpeech: {
      noun: "Noun",
      verb: "Verb",
      adjective: "Adjective",
      adverb: "Adverb",
      pronoun: "Pronoun",
      particle: "Particle",
      determiner: "Determiner",
      numeral: "Numeral",
      interjection: "Interjection",
      phrase: "Expression",
      other: "Word",
    },
  },
  streakDay: {
    eyebrow: "Streak",
    title: "Amazing!",
    subtitle: "Keep practicing every day\nto continue your streak",
    continue: "Continue",
  },
  scoreUp: {
    eyebrow: "SCORE UP",
    title: "You reached score {{score}}!",
    sub: "You finished unit {{unit}} completely. Keep this pace and the next section is close.",
    continue: "Continue",
  },
  jump: {
    passTitle: "Unit {{unit}} unlocked!",
    passSectionTitle: "Section {{section}} unlocked!",
    passSectionSub: "You passed. Pick up right at section {{section}}.",
    passUnitTitle: "Unit {{unit}} unlocked!",
    passUnitSub: "You passed. Pick up right at unit {{unit}}.",
    unlockedSectionLabel: "SECTION UNLOCKED",
    unlockedUnitLabel: "UNIT UNLOCKED",
    skippedLessons: "You skipped {{count}} lessons",
    wrongCount: "You missed {{count}} questions",
    goStudy: "Start learning",
    passSub: "You passed the test! You can now continue from here.",
    failTitle: "A bit more practice needed",
    failSub:
      "Too many mistakes. Build your skills by completing earlier lessons!",
    continue: "Continue",
    introRule:
      "To pass this challenge, finish with fewer than {{count}} mistakes. Good luck!",
    introContinue: "Continue",
    startTitle: "Jump to Unit {{unit}} and prove your skills!",
    startBtn: "Start test",
    later: "Maybe later",
    // en.ts
    startTitleSection: "Pass this test to skip ahead to Section {{section}}.",
  },
  legend: {
    introTitle: "Prove your legendary skills! Plus, earn extra rewards.",
    start: "Start +{{xp}} XP",
    later: "Maybe later",
  },
  writePractice: {
    retryHint: "Not quite. Check the hint and type it again.",
    correctAnswer: "Correct answer",
    level: "Level {{n}}",
    hint: "Hint",
    voiceMode: "Voice",
    confirm: "Done",
    retry: "Retry",
    isNew: "New",
    reviewInDays: "Review in {{count}}d",
    reviewInYears: "Review in {{count}}y",
    wrongDefault: "Check the answer and type it again.",
    source: "[Source] {{name}}",
    otherExample: "Examples",
    wrongNote: "Mistakes",
    aiQa: "Ask AI",
    wordInfo: "Word",
    pronunciation: "Pronounce",
    realSpeed: "Real speed",
    listenAgain: "Replay",
    nextQuestion: "Next",
  },
  pronQuiz: {
    perQuestion: "{{pts}} points per question",
    hardUnlocked: "HARD mode unlocked!",
    hardNeed: "Get {{need}} right to unlock HARD",
    hardDone: "HARD finished too. Stage complete!",
    practice: "Try saying it",
    lecture: "Lesson",
    question: "Which word did you just hear?",
    listenOnly: "Listen and choose",
    next: "Next",
    done: "Done",
    noData: "This step isn't ready yet.",
    resultCount: "{{correct}} of {{total}} correct",
    saving: "Saving your progress…",
    saved: "Progress saved",
    saveFailed: "Couldn't save your progress",
    retrySave: "Try saving again",
  },
  sentenceBuild: {
    wrongHere: "'{{word}}' does not fit here. Try another word.",
    title: "Build the sentence",
    level: "Level {{n}}",
    checkAnswer: "Check answer",
    minAgo: "{{count}} min ago",
    reviewInDays: "Review in {{count}}d",
    isNew: "New",
    wrongDefault: "That word is off. Which one fits better?",
    otherExample: "Examples",
    wrongNote: "Mistakes",
    speak: "Speak",
    dictSearch: "Dictionary",
    sentenceDiagnosis: "Pronunciation",
    exprDict: "Phrases",
    listenAgain: "Replay",
    nextQuestion: "Next",
  },
  pronPractice: {
    title: "Pronunciation Practice",
    tabs: { lv1: "Lv.1", lv2: "Lv.2", lv3: "Lv.3", lv4: "Lv.4" },
    stories: {
      lv1: "Haneulmon's first order",
      lv2: "Haneulmon goes to the market",
      lv3: "Haneulmon asks for directions",
      lv4: "Haneulmon's first presentation",
    },
    focus: {
      lv1: "Plain vs aspirated",
      lv2: "Plain vs tense",
      lv3: "Confusing vowels",
      lv4: "Final consonants",
    },
    step: "Step {{n}}",
    front: "Word-initial",
    back: "Word-final",
    start: "START",
    hardLocked:
      "Get {{need}} of {{total}} right on EASY to unlock HARD. ({{score}}+ points)",
    confirm: "OK",
    progressLoadFailed: "Couldn't load your saved progress",
  },
  expressionHub: {
    title: "Expressions",
    subtitle: "Useful Korean for real situations",
    currentRange: "Current study range",
    range: "Section {{section}} · Unit {{unit}}",
    change: "Change",
    continueEyebrow: "CONTINUE LEARNING",
    continueDescription: "Learn expressions you can use right away.",
    packProgress: "{{done}} / {{total}} expressions",
    continueButton: "Continue",
    startButton: "Start pack",
    reviewTitle: "Today's review",
    reviewCount: "{{count}} due",
    reviewDescription: "Review before you forget",
    savedTitle: "Saved expressions",
    savedCount: "{{count}} saved",
    savedDescription: "Find them whenever you need",
    situationTitle: "Expressions by situation",
    situationDescription: "Start with the situation you need now.",
    packCount: "{{count}} packs",
    expressionCount: "{{count}} expressions",
    loading: "Loading expressions",
    loadFailed: "Couldn't load expressions",
    empty: "There are no expressions in this study range yet",
  },
  expressionRoadmap: {
    title: "Expression roadmap",
    subtitle: "Learn every situation from start to finish",
    loading: "Loading the expression roadmap",
    loadFailed: "Couldn't load the expression roadmap",
    retry: "Try again",
    empty: "There are no expression nodes to study yet",
    overallProgress: "Overall learning progress",
    heroTitle: "Complete Korean for real situations",
    heroDescription:
      "See every expression three times to unlock the next node.",
    progressMeta: "{{learned}} of {{total}} expressions learned",
    nodeExpressions: "{{count}} expressions",
    nodeRepetitions: "{{count}} times",
    startNode: "Start expressions",
    reviewNode: "Review expressions",
    openTopicList: "View all expression topics",
    closeTopicList: "Close expression topics",
    topicListTitle: "All expression topics",
    topicListSubtitle: "Choose a situation to jump to its place on the roadmap.",
    topicListCurrent: "Current",
    topicListProgress: "{{completed}}/{{total}} nodes",
  },
  expressionLearning: {
    listen: "Listen again",
    meaning: "Meaning",
    speaker: "Usually said by",
    anySpeaker: "Anyone",
    context: "When it is used",
    usageNote: "How to sound natural",
    detailsOpen: "Show expression details",
    detailsClose: "Hide details",
    practiceOfferEyebrow: "You've practiced every expression",
    practiceOfferTitle: "Ready to recall them?",
    practiceOfferDescription:
      "Check your memory by saying or typing the {{count}} expressions you just practiced from their meanings.",
    practiceOfferStart: "Start expression practice",
    practiceOfferLater: "Maybe later",
    practice: {
      stage: {
        learn: "Step 1 · Learn",
        guided: "Step 2 · Repeat",
        recall: "Step 3 · Recall",
      },
      speakPrompt: {
        learn: "Listen, then try saying it yourself",
        guided: "Look at the expression and say it naturally",
        recall: "Look at the meaning and say it in Korean",
      },
      typePrompt: "Complete the Korean expression",
      speakMode: "Speaking practice",
      typeMode: "Quiet typing",
      recallPrompt: "Recall the Korean expression from its meaning",
      passed: "That sounded natural!",
      needsPractice: "Let's polish this one more time",
      typePlaceholder: "Type in Korean",
      tryAgain: "Not quite. Check it and try again",
      correct: "Correct!",
      hint: "Hint",
      check: "Check answer",
      later: "Practice this again later",
      completeToContinue: "Practice now or choose to repeat it later",
    },
    loadFailed: "Couldn't load this expression lesson",
    retry: "Try again",
    saveFailed: "Couldn't save your progress. Tap again.",
    next: "Next expression",
    finish: "Complete node",
    doneTitle: "Expression node complete!",
    doneDescription: "You viewed all {{count}} expressions in ‘{{node}}’.",
    backToRoadmap: "Back to roadmap",
  },
  expressionPack: {
    learning: "EXPRESSION CARDS",
    swipeHint: "Swipe left or right to study each expression",
    singleExpression: "One at a time",
    meaning: "Meaning",
    context: "When to use it",
    listen: "Listen to expression",
    save: "Save expression",
    removeSaved: "Remove saved expression",
    previous: "Previous expression",
    next: "Next expression",
    loading: "Loading expression cards",
    loadFailed: "Couldn't load the expression cards",
    empty: "There are no expressions to study in this pack yet",
    retry: "Try again",
    startPractice: "Start practice",
    speechLevel: {
      polite: "Polite",
      casual: "Casual",
      formal: "Formal",
    },
  },
  // en.ts
  aiChat: {
    correctionTitle: "Grammar fix",
    teacherName: "Haneulmon",
    online: "Online · AI conversation tutor",
    inputPlaceholder: "Type in Korean...",
    today: "Today",
    emptyHint: "Practise Korean with Haneulmon.\nStart with anything you like!",
    errorReply: "Couldn't send your message. Please try again in a moment.",
  },
  grammarPractice: {
    entryTitle: "Grammar practice",
    entrySub: "Practise what you've learned with blanks and sentence building",
    empty: "No grammar to practise yet.\nLearn some grammar first.",
    finished: "Grammar practice complete!",
    back: "Back",
  },
  grammarList: {
    title: "Grammar",
    hero: "Master grammar step by step, section by section.",
    section: "Section {{n}}",
    dayTitle: "Day {{n}} grammar",
    dayHero: "Today's grammar.",
    count: "{{count}} points",
    locked: "Finish the previous section",
    sectionDone: "Done!",
    comingSoon: "Coming soon",
    superOnly: "KORIO SUPER",
    premiumTitle: "Keep learning with SUPER",
    premiumDesc:
      "You've finished the free sections! Unlock all grammar with KORIO SUPER.",
    premiumCta: "Start SUPER",
    premiumLater: "Maybe later",
  },
  grammarStudy: {
    finishDay: "Today's grammar done! Back to the map →",
    crumb: "Grammar · Progressive",
    learning: "Learning",
    explanation: "Explanation",
    conjugation: "How it attaches",
    examples: "Examples",
    dialogue: "In conversation",
    similar: "Similar grammar",
    cautions: "Watch out",
    practice: "Practice",
    correct: "Correct! ✓",
    tryAgain: "Try again!",
    doneTitle: "Done! 🎉",
    doneSub: "You finished this grammar practice",
    retry: "Retry",
    next: "Next grammar",
  },
  errors: {
    title: "Oops, something went wrong",
    close: "Close",
    NETWORK_ERROR: "Please check your internet connection.",
    SERVER_ERROR: "Something went wrong on the server. Please try again soon.",
    UNAUTHORIZED: "Please log in again.",
    FORBIDDEN: "You don't have permission.",
    NOT_FOUND: "We couldn't find what you requested.",
    TIMEOUT: "The request timed out. Please try again.",
    UNKNOWN_ERROR: "An unknown error occurred. Please try again.",
    retry: "Try again",
  },
  avatarEditor: {
    title: "My character",
    subtitle: "Create a KORIO character that feels like you",
    allFree: "All items are free for now",
    randomize: "Random",
    save: "Save character",
    open: "Edit avatar",
    close: "Close avatar editor",
    unsavedTitle: "Discard your changes?",
    unsavedMessage: "Your unsaved character changes will be lost.",
    discard: "Discard",
    saving: "Saving...",
    saveFailedTitle: "Couldn’t save",
    saveFailedMessage: "Check your internet connection and try again.",
    categories: {
      skinTone: "Skin",
      bodyShape: "Body",
      expression: "Face",
      eyeColor: "Eyes",
      hairstyle: "Hair",
      hairColor: "Hair color",
      eyewear: "Glasses",
      facialHair: "Facial hair",
      headwear: "Hats",
      outfit: "Outfit",
      background: "Background",
    },
  },
  arcade: {
    title: "Arcade",
    wordRain: "Word Rain",
    swipeJudge: "Swipe Judge",
    particleRush: "Particle Rush",
    echoChain: "Echo Chain",
    gameOver: "Game Over!",
    score: "Score",
    caught: "Words caught",
    bestCombo: "Best combo",
    accuracy: "Accuracy",
    correctCount: "Correct",
    maxRound: "Best round",
    round: "Round",
    fever: "FEVER",
    swipeRight: "Correct",
    swipeWrong: "Wrong",
    listenCarefully: "Listen carefully...",
    yourTurn: "Tap in order!",
    playAgain: "Play again",
    exit: "Exit",
  },
  studyMode: {
    guided: "Study path",
    free: "Free study",
  },
  studyModeModal: {
    title: "How do you want to learn?",
    subtitle: "Starting {{course}}. You can switch any time.",
    hint: "Not sure? Start with the study path.",
    guided: {
      title: "Study path",
      desc: "In order, like a class. We decide what you do today.",
    },
    free: {
      title: "Free study",
      desc: "Wherever you like. Pick your own way through the map.",
    },
  },
  studyLevel: {
    title: "Where should we start?",
    subtitle:
      "Pick what matches your Korean right now.\nYou can change it any time.",
    comingSoon: "Coming soon",
    hint: "Not sure? Starting from Level 1 is a safe bet.",
    change: "Change level",
  },
  levelExam: {
    cardTitle: "Level {{n}} final exam",
    cardBody: "25 questions on everything from this level.",
    cardDone: "Already passed. You can take it again.",
    passedTitle: "Level {{n}} passed!",
    passedBody: "Getting here is no small thing. On to the next level.",
    missedTitle: "Level {{n}} — so close",
    missedBody:
      "The next level is open anyway. Another look at these will make it much easier.",
    score: "{{correct}} of {{total}} correct",
    weakTitle: "These gave you trouble",
    nextOpen: "Level {{n}} is now open",
    continue: "Continue",
    retry: "Try again",
    area: {
      vocabulary: "Vocabulary",
      grammar: "Grammar",
      expression: "Phrases",
      conversation: "Conversation",
      listening: "Listening",
      etc: "Other",
    },
  },
  studyPath: {
    sectionStart: "Section {{n}} begins",
    levelShort: "Lv.{{n}}",
    lockedHint: "Opens once you finish the step before",
    preview: "Preview",
    lessonProgress: "Ring {{done}}/{{total}}",
    continueLesson: "Continue from {{n}}",
    nodeGroup: "{{name}} {{n}}/{{total}}",
    dayLearn: "Day {{n}} · Learn {{title}}",
    dayPractice: "Day {{n}} · Practice {{title}}",
    phaseLearn: "Day {{n}} · Learning",
    phasePractice: "Day {{n}} · Practice",
    title: "Study path",
    dayLabel: "Day {{n}}",
    dayComplete: "Today's lesson done!",
    step: "Step {{step}} / {{total}}",
    progressOf: "{{done}}/{{total}}",
    start: "Start",
    again: "Do again",
    countWords: "{{count}} words",
    countGrammar: "{{count}} grammar points",
    countQuestions: "{{count}} questions",
    loadFailed: "Couldn't load your study path.",
    retry: "Try again",
    empty: "No lessons are ready yet.",
    node: {
      review: "Review last unit",
      words: "Today's words",
      grammar: "Today's grammar",
      vocabQuiz: "Vocabulary practice",
      recap: "Yesterday's review",
      grammarQuiz: "Grammar practice",
      final: "Final check",
    },
    desc: {
      review: "Start by going back over the last unit.",
      words: "Learn today's words with cards.",
      grammar: "Learn today's grammar rule.",
      vocabQuiz: "Put the vocabulary into questions.",
      recap: "Go over what you learned yesterday.",
      grammarQuiz: "Put today's grammar into questions.",
      final: "Revisit what you missed and finish this unit.",
    },
  },
};
